...
const Leaderboard = () => {
  ...
  const [me, setMe] = useState<XpRow | null>(null);
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      setLoading(true);
      try {
        // XP leaderboard for current month
        const now = new Date();
  ...
  const myRank = useMemo(() => {
    if (!user) return null;
    const found = rows.find(r => (r as any).user_id === user.id);
    return (found as any)?.rank ?? null;
  }, [rows, user]);
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-indigo-50 via-purple-50 to-pink-50">
      <div className="container mx-auto px-4 py-10 max-w-3xl">
        ...
        {/* XP is the only leaderboard per HR spec */}
        ...
        {guest || !user ? (
          ...
          <>
            ...
            <Card>
              <CardHeader>
                <CardTitle className="text-lg">Your XP {myRank ? `(Rank #${myRank})` : "(Outside top 50)"}</CardTitle>
              </CardHeader>
              <CardContent>
